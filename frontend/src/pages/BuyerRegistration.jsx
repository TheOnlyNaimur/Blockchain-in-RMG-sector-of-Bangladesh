...
import { useBuyerRegistration } from "../hooks";

export default function BuyerRegistration() {
  const navigate = useNavigate();
  const { address } = useAccount();
  const { data: walletClient } = useWalletClient();
  const { execute: registerBuyer, loading, error } = useBuyerRegistration();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const formData = { name, license, contact };
      ...
      localStorage.setItem(
        ...
        JSON.stringify({ role: "buyer", profile: { name } }),
      );
          ...
          <div className="p-6 border-b border-border-dark flex justify-between items-center bg-surface-darker">
            <div>
              ...
              <p className="text-text-secondary text-sm mt-1">
                Complete your profile to start purchasing on the blockchain
                network.
              </p>
            </div>
            <Link
